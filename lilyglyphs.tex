...
As you can see the glyph is -- as most \emph{Emmentaler} glyphs are -- placed too low, so you have to place it in a  \cmd{raisebox}: \raisebox{0.25em}{\lilyGlyph[1.3]{"scripts.coda"}}, finding out the offset value by trial-and-error: \cmd{raisebox\{0.25em\}\{\textbackslash lilyGlyph\{"scripts.coda"\}\}}. The syntax is still not fixed yet, because in a future version of the package the \cmd{raisebox} will be a direct part of the \cmd{lilyGlyph} command, with the raise amount being an optional argument.
