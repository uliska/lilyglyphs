...
The key=value mechanism is achieved using the \texttt{keyval} package as the most basic solution available.
...
\bigskip
The next step are the three generic access functions \cmd{lilyGlyph}, \cmd{lilyGlyphByNumber} and \cmd{lilyText}.
They are very similar and differ only in the way they determine the actual content to be printed.
As stated in the end user part of this documentation they expect two arguments, the optional key=value pair list and the contents.
...
\noindent Commands that print Emmentaler glyphs:
