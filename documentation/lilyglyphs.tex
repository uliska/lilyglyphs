...
\section*{Quick Start}
\lilyglyphs{} allows to include notational elements from the LilyPond%
\footnote{\url{http://www.lilypond.org}}
notation software like characters in the continuous text of \LaTeX{} documents.
For inserting full-fledged music examples we also provide the \package{musicexamples} package%
\footnote{\url{http://www.openlilylib.org/musicexamples}}.

\lilyglyphs{} relies on \hologo{XeLaTeX} or \hologo{LuaLaTeX} to compile documents because it accesses glyphs from an OpenType font through the \package{fontspec} package.
...
\flatflat{} or \clefGInline. % Predefined commands, 
                             % Use {} to allow traling whitespace
...
Glyphs accessed individually need these options in general, the above neomensural time signature having actually been scaled by 1.2 and raised by 0.5.
...
There are several ways to get \lilyglyphs{} up and running.
If you have got it as part of your \LaTeX{} distribution you should be able to simply \emph{use} the package and start typing.
If your distribution doesn't contain \lilyglyphs{} yet you can download the package from CTAN%
\footnote{URL not yet known.}  
or from openLilyLib's website%
\footnote{\url{http://www.openlilylib.org/lilyglyphs}}.
In this case it should be enough to simply unpack the downloaded archive to a directory within your \package{texmf} tree and possibly run \texttt{texhash} -- please refer to the documentation of your distribution for details on making packages available.

The recommended -- although slightly more involved -- way to get \lilyglyphs{} is to clone or fork the GitHub repository at \url{https://github.com/openlilylib/lilyglyphs}.
If you are completely sure that you'll never contribute to the package then cloning directly will be somewhat more straightforward for you.
But if there is any chance you might someday contribute something it is a better idea to first fork the repository, clone your personal fork and then add the original repository as an additional remote.
Please note that in our opinion intense use of the package quite naturally leads to contributing.

\todo{Does one have to uninstall the distribution version of the package in order to use the Git one?
Or does a package in the personal \texttt{texmf} tree “hides” the distribution's one?}
...
Each time someone adds new items to the list of commands they will be instantly accessible through the repository while regular releases will happen much less frequently.
And if updates should somehow break things in existing documents you can easily (and temporarily) check out the earlier version of the package you had used when originally writing your documents.
Another advantage of the repository version is that you have “direct access” to extending the package with predefined commands in a way that makes it for us easy to incorporate your contribution in the official package.

If you haven't got \lilyglyphs{} through your \LaTeX{} distribution you may have to take care that you use a sufficiently current distribution.
Development of \package{fontspec} and \hologo{LuaLaTeX} are quite active, and we know that for running \lilyglyphs{} with \hologo{LuaLaTeX} fairly recent versions of its packages are necessary. 
If you experience any issues with this or other topics during installation please give us feedback so we can improve our documentation.
...
As mentioned in the Quick Start at the beginning of this document you have to activate \lilyglyphs{} with \cmd{usepackage\{lilyglyphs\}}.
...
You can look up the glyph names in the Appendix of LilyPond's \emph{Notation Reference}\footnote{\url{http://www.lilypond.org/doc/v2.16/Documentation/notation/the-feta-font}} or in the somewhat reduced html page provided in the \dir{/glyphlist} directory of the package download. 
...
This only works for Dynamics letters, numbers and the glyphs \lilyText[scale=2]{+ - , .} -- as these glyphs are located at their ordinary character position in the font. 
...
\todo{Verify this. Would that apply to the faulty downways half note rests?}
...
\cmd{newcommand\{\textbackslash ufermata\}\{hspace\{1ex\}}\cmd{lilyGlyph[scale=1.4,raise=0.3]\{scripts.ufermta\}}.
Then you can enter your tweaked symbol by simply writing \cmd{ufermata}.
...
The documentation explicitely mentions if the commands are are based on image files.
...
But as long as it isn't possible to make this parametrical and draw them directly from within \lilyglyphs{} it is probably a good idea to stick with a few basic commands.
However it would be quite simple to create “plugin modules” that provide a series of related symbols, and maybe we will think about such an extension in the future.
...
The numerical (single and compound) time signatures can be printed using \cmd{lilyTimeSignature}: \lilyTimeSignature{4}{4}.
This command expects two mandatory arguments: numerator and denominator.
They are treated as \cmd{lilyText}, so you can enter anything this command expects (see \fref{sec:generic_access_commands}).
This way you can easily write compound time signatures like \cmd{lilyTimeSignature\{4 + 7\}\{8\}}: \lilyTimeSignature{4 + 7}{8}.
But be aware that the command does \emph{not} have a notion of columns, so you have to take care about the horizontal alignment yourself if there are more than one item in both rows, for example by adding explicite space.
...
\medskip
In order to make the package's \texttt{.sty} file easier to understand, its content is split into multiple input files which are located in the \texttt{/commands} and \texttt{/core} subfolders.
...
\medskip
At the next higher level there are the three generic access functions \cmd{lilyGlyph}, \cmd{lilyGlyphByNumber} and \cmd{lilyText}, defined in \texttt{genericAccess.inp}.
...
This section is relevant if you want to write new commands on a regular basis, and especially if you want to contribute your results back to the package or if you want to use the Python scripts as described in \fref{chap:python_scripts}.

If you are using \lilyglyphs{} from a \TeX{} distribution you shouldn't really consider the contents of the directory structure: probably you don't have write access there anyway, and any updates of the package or the distro might overwrite your manual additions without warning.
Please see \fref{chap:contributing} for recommendations how to set up your installation.
...
contains files that define the glyph commands that are available to the end user of \lilyglyphs.
If you are going to write new commands you will include them in these files or add new files here.

\paragraph*{\dir{/otf}}
contains the \emmentaler{} font files, copied from LilyPond \todo{enter version number}.
...
contains the manual, its source and helper file(s).
Of particular interest in this directory may be the \dir{glyphlist} folder containing an overview of the \emmentaler{} glyphs and the \dir{lilyglyphs\_logo} providing the \lilyglyphs{} logo as \textsc{pdf} and \textsc{png} files.

\begin{itemize}
\item \todo{Think about stash\_new\_commands}
\end{itemize}

\subsection{“Private” Directory Structure}
\label{subsec:private_directory_structure}
If you have obtained \lilyglyphs{} any other way than through its Git repository you probably don't want to (or can't) modify the package files directly.
Instead we recommend you set up a private “shadow” directory structure in your \LaTeX{} search path where you can safely place your own additions.
A good choice for example would be \dir{\textasciitilde/texmf/tex/latex/lilyglyphs-private}.
We have prepared such a structure in the \texttt{lilyglyphsPrivate.tar.gz} file in the root directory.
If you extract this archive to the suggested path you should be ready to go.
...
The first file is a \LaTeX{} package where you can store and organize your own predefined commands, the second a file where you can (and are strongly advised to) document your additions.
...
Currently this approach is very limited because the Python scripts are only designed to work within the original package directory, but we'll soon update this -- and then we'll extend the current documentation too.
Please also see the chapter \fref{chap:contributing} on contributing.
...
This example is meant to encourage you to experiment with the definition of new commands. All you have to deal with is setting the design time options and the optional argument, and chosing the appropriate input method for the second argument.

\bigskip
Once you have defined your new command you of course have to make it available to \LaTeX.
...
If it is an image driven command you will have created the image file before and will have to place it in a location where \LaTeX{} can find it.
...
As we have seen in \fref{subsubsec:commands_single-glyphs} it is quite easy to create predefined commands.
...
\todo{I assume that the path issue is updated according to my Trello card.}

The first step is to create a text file with entries for any number of new commands to be created.
You can save it to any convenient location, and it is up to you to keep this file for reference after use.
...
The end of the definition is indicated by an empty line, therefore it is important that your file ends with a complete empty line, otherwise the last entry will be discarded.
...
The order doesn't matter, but it is advisable to stick to one style.
...
\begin{itemize}
	...
	The possible kind of values depends on the type of the command, as described below.
	...
	\begin{itemize}
		...
		\item \textbf{\texttt{text:}} The content is passed as plain text (works only for Dynamic letters, numbers and + - , .)
		...
		This is the preferred way to create such a command if you already have an image file obtained from or created with any other source than LilyPond (e.\,g. a scanned image from a printed edition or an autograph).
...
The Python script that processes your input file is \texttt{genGlyphCommands.py}, located in the \dir{/py} subdirectory of the package.
You simply pass it the (absolute or relative) filename of the input file as its first parameter.
The program parses the input file and creates a new file, by default one with the same basename but a \texttt{.tex} extension, in the same folder where the input file is.

\todo{Make sure this is true and correct. Add and document new options for the script}
...
\todo{Insert examples how the both examples from the previous sections would be processed.}
...
If you are working on a fork of the GitHub repository (as is recommended for potential participators) you can incoporate the new commands directly and send us a pull request (preferably as soon as possible to minimize risk of conflicts with others' changes).
...
For any new files you might create copy\,\&\,paste the usual copyright comment at the beginning from another file.
...
Creating commands with images generated by LilyPond is a significantly more complex task than simply inserting Emmentaler glyphs, and therefore the Python script \texttt{buildglyphimages.py}, located in the \texttt{/py} directory, is a more complex one.
...
\texttt{buildglyphimages.py} doesn't expect regular LilyPond source files as its input, but rather a file with one or multiple ‘snippets’ in it, similar to those for \texttt{genGlyphCommands.py}. 
...
The file name will also be reflected in auto-generated directory names so you should consider the naming thoroughly.
...
A command entry starts with the special key \verb|%%lilyglyphs|.																																																					
...
You should use this key whenever you consider a command finished because it will prevent LilyPond from recompiling the command%
...
This will also end the command entry, but for readability's sake it is surely a good idea to enter at least one empty line after the entry.
...
\todo{What is the way to create images \emph{with} these elements? IS there a way? If not refer to a future version.}
...
\todo{Update the script to be more versatile with directories.
In particular it should work on the package subdir or the private dir.
When updated document correct behaviour.}
...
For each command definition it generates one compilable LilyPond source file.
This will be named according to the command name and put in a subdirectory of \dir{/generated\_src}, named according to the input file.
So if you combined several glyphs in a file \texttt{singlenotes.ly} the results will be stored in a \dir{singlenotes} subdirectory.
...
Commands that have been processed earlier and that aren't marked as protected will overwrite the existing LilyPond file.

\todo{Completely review this file name behaviour.
The problem is that it doesn't prevent name clashes reliably.
Even if I have a check in the script this can be tricked by manual processes, it would be better to have a control mechanism on filesystem level.
So it would be better to have all commands in one directory (i.\,e. one generated.. dir and one pdf dir, without subdirs.
The downside is that this can become quite crowded if \emph{all} commands live in the same directory.
OTOH there exist plenty of applications with subdirs with thousands of items.)}
...
The directory is then cleaned up, and the resulting small PDF files are moved to the corresponding subdirectory below \dir{/pdfs}.
...
\todo{Think about where this is stored. Implement the change in the script.}
...
\todo{Review when script has been updated}

You shouldn't touch the first two items but go on and open the \texttt{.tex} file.
This file will in fact be the same as one generated by \texttt{genGlyphCommands.py}, and also the way to finish off and possibly contribute your commands is essentially the same as described in \fref{subsec:GGC_fine-tuning} and \fref{subsec:GGC_finishing_off}.

There are a few differences however:

\begin{itemize}
\item You should always keep the original definitions file so you can later re-generate the image from it.
This can happen when you need to modify it -- or when you want to upgrade the package to a new version of \emmentaler.
\item Don't forget to mark finished command definitions as ‘protected’.
\item Keep in mind that this file will be overwritten by the next time the same input file is processed by the script, so if you need to make any changes to the \LaTeX{} command definitions you should immediately copy them to a safe place.
...
One example would be the upgrade to a new \emmentaler{} version.
...
Fortunately I realized very early that I should make all this more generic and create a public package.
...
By now I have a solution that completely works for my personal needs and can be a useful package for any \LaTeX{} authors interested in incorporating musical symbols in their documents.
...
I personally will probably only add new commands as I need them for my work, maybe sometimes get the hang and complete a group.
...
And finally I have some ideas how \lilyglyphs' functionality could be enhanced, and others could come with other ideas.
