...
Creating commands with images generated by LilyPond is a significantly more complex task than simply inserting Emmentaler glyphs, and therefore the Python script \texttt{genImageCommands.py}, located in the \texttt{/py} directory, is a more complex one.
...
\texttt{genImageCommands.py} doesn't expect regular LilyPond source files as its input, but rather a file with one or multiple ‘snippets’ in it, similar to those for \texttt{genGlyphCommands.py}. 
...
If you have prepared your input file you can run the \texttt{/py/genImageCommands.py} script, passing it the complete file name (as a relative or absolute path) of your definitions file.
...
If there is such a file already (the command had been processed before) you will be asked if you want to overwrite this file.
...
This file is structured identically as the files generated by \texttt{genGlyphCommands.py}, and also the way to finish off and possibly contribute your commands is essentially the same as described in \fref{subsec:GGC_fine-tuning} and \fref{subsec:GGC_finishing_off}.
