...
\section{Generic Access Ccommands}
...
If you simply want to add your own predefined commands “the \lilyglyphs{} way” it is a good idea to read this section too, but you may also skip it and directly go to \fref{sec:howto_predefined_commands}.
...
This can be a file in \texttt{.pdf}, \texttt{.png} or \texttt{.jpg} format, but we highly recommend using \textsc{pdf} files for sake of printing quality.
...
This example is meant to encourage you to experiment with the definition of new commands. It is pretty much the same as usual when inventing new commands. You just have to deal with the design time options, the optional argument and getting the correct input as the second argument.
...
Once you have defined your new command you of course have to make it available to \LaTeX.
If it is a “local” command that you just use for a specific document you can simply put it in the document's preamble or a dedicated helper \texttt{.sty} file.
Of course if it is an image driven command you will have created the image file before and will have to place it in a location where \LaTeX{} can find it.
For more information on how to publicly contribute your new commands to the package see \fref{chap:contributing}.
...
And it is nevertheless a good idea to have read \fref{sec:howto_predefined_commands} to have a good understanding of what is going on.
